...
from global_functions import test_stationarity, calculate_optimal_pdq_arima, calcul_nbr_lags
...
def prediction_auto(df):
    ...
    if st.session_state.compute_auto:
        ...
        if not test_stationarity(df['value']):
            ...
            auto_p, auto_d, auto_q = calculate_optimal_pdq_arima(df_diff, "value")
            st.write(f"Les valeurs de p, d et q sont respectivement: {auto_p}, {auto_d} et {auto_q}")
            ...
            with st.spinner("Entraînement <<auto>> du modèle (différentié) en cours..."):
                ...
                #modelisation
                model = ARIMA(train, order=(auto_p,auto_d,auto_q))
                model_fit = model.fit()
                ...
                full_model = ARIMA(df_diff, order=(auto_p,auto_d,auto_q))
                ...
                ax.set_title(f"Prédictions automatiques <<différentielles>> avec le modèle ARIMA(p={auto_p}, d={auto_d}, q={auto_q})")
                ax.legend()
                st.pyplot(fig)
            ...
            auto_p, auto_d, auto_q = calculate_optimal_pdq_arima(df, "value")
            st.write(f"Les valeurs de p, d et q sont respectivement: {auto_p}, {auto_d} et {auto_q}")
            ...
            with st.spinner("Entraînement <<auto>> du modèle (NON différentié) en cours..."):
                ...
                #PARTIE 2 : PRÉDICTIONS (20% avec 100% des données)
                #ré-entraînement sur TOUTE la série différenciée
                full_model = ARIMA(df, order=(auto_p,auto_d,auto_q))
                full_model_fitted = full_model.fit()
                ...
                ax.axvline(df.index[train_size], alpha=0.6, linestyle=':', color="gray", label='Train/Test split')
                ax.axvline(df.index[-1], alpha=0.6, linestyle=':', color="black", label='Next data')
                ax.set_title(f"Prédictions automatiques <<non différentiées>> avec le modèle ARIMA(p={auto_p}, d={auto_d}, q={auto_q})")
                ax.legend()
                st.pyplot(fig)
...
def prediction_manuel(df):
    ...
    if st.session_state.compute_manuel:
        ...
        if not test_stationarity(df['value']):
            ...
            user_p = int(user_p)
            user_d = st.slider("Nombre (d) pour différentiation(s)", min_value=min, max_value=max, value=min, step=1)
            st.write(f"Le nombre d choisi est : {user_d}")
            user_d = int(user_d)
            ...
            if st.button("Exécuter"):
                ...
                with st.spinner("Entraînement <<manuel>> du modèle (différentié) en cours..."):
                    ...
                    test = df_diff.iloc[train_size:]
                    #modelisation
                    model = ARIMA(train, order=(user_p,user_d,user_q))
                    model_fit = model.fit()
                    #predictions différentiées
                    pred_train = model_fit.predict(start=train.index[0], end=train.index[-1], dynamic=False)
                    pred_test = model_fit.predict(start=test.index[0], end=test.index[-1], dynamic=False)
                    ...
                    #PARTIE 2 : PRÉDICTIONS (20% avec 100% des données)
                    #ré-entraînement sur TOUTE la série différenciée
                    full_model = ARIMA(df_diff, order=(user_p,user_d,user_q))
                    full_model_fitted = full_model.fit()
                    n_future = int(len(df) * 0.2)  # 20% de la série originale
                    pred_future_diff = full_model_fitted.predict(start=len(df_diff), end=len(df_diff) + n_future - 1)
                    #inverse de la différenciation sur les valeurs futures
                    ...
                    ax.axvline(df.index[train_size], alpha=0.6, linestyle=':', color="gray", label='Train/Test split')
                    ax.axvline(df.index[-1], alpha=0.6, linestyle=':', color="black", label='Next data')
                    ax.set_title(f"Prédictions automatiques <<différentielles>> avec le modèle ARIMA(p={user_p}, d={user_d}, q={user_q})")
                    ax.legend()
                    st.pyplot(fig)
            ...
            if st.button("Exécuter"):
                ...
                with st.spinner("Entraînement <<manuel>> du modèle (NON différentié) en cours..."):
                    ...
                    #PARTIE 2 : PRÉDICTIONS (20% avec 100% des données)
                    #ré-entraînement sur TOUTE la série 
                    full_model = ARIMA(df, order=(user_p,user_d,user_q))
                    full_model_fitted = full_model.fit()
                    #calcul des 20% FUTUR (basé sur la taille totale des données)
                    n_future = int(len(df) * 0.2)  # 20% de la série future
                    pred_future = full_model_fitted.predict(start=len(df), end=len(df) + n_future - 1)
                    ...
                    ax.axvline(df.index[train_size], alpha=0.6, linestyle=':', color="gray", label='Train/Test split')
                    ax.axvline(df.index[-1], alpha=0.6, linestyle=':', color="black", label='Next data')
                    ax.set_title(f"Prédictions automatiques <<non différentiées>> avec le modèle ARIMA(p={user_p}, d={user_d}, q={user_q})")
                    ax.legend()
                    st.pyplot(fig)
...
def prediction_pmdarima(df):
    "Prévision 100% automatique avec pmdarima"
    if 'compute_manuel' not in st.session_state:
        st.session_state.compute_manuel = False
    ...
    if st.session_state.compute_manuel:
        with st.spinner("Entraînement du modèle en cours..."):
            ...
            #predictions
            pred_train, train_confint = model_fit.predict_in_sample(return_conf_int=True)
            n_periods_test = len(test)
            pred_test, test_confint = model_fit.predict(n_periods = n_periods_test, return_conf_int=True)
            #train 
            forecast_train = pred_train
            #test
            forecast_test = pred_test

            #PARTIE 2 : PRÉDICTIONS futures (20% avec 100% des données)
            #ré-entraînement sur TOUTE la série différenciée
            full_model = auto_arima(df, start_p=0, start_d=0, start_q=0,
                    max_p=max_p, max_d=max_d, max_q=max_q,
                    seasonal=True, trace=True,
                    error_action='ignore', suppress_warnings=True)
            full_model_fit = full_model.fit(df)
            #calcul des 20% FUTUR (basé sur la taille totale des données)
            n_future = int(len(df) * 0.2)  # 20% de la série future
            pred_future, confit_future = full_model_fit.predict(n_periods = n_future, return_conf_int=True)
            #future
            forecast_future = pred_future
            ...
            ax.axvline(df.index[train_size], alpha=0.6, linestyle=':', color="gray", label='Train/Test split')
            ax.axvline(df.index[-1], alpha=0.6, linestyle=':', color="black", label='Next data')
            ax.set_title(f"Prédictions automatiques avec le modèle ARIMA(pmdarima)")
            ax.legend()
            st.pyplot(fig)
            ...
            st.write(f"Racine de l'Erreur Quadratique Moyenne (RMSE) : {rmse_auto:.3f}")
            st.write(f"Erreur Absolue Moyenne (MAE) : {mae_auto:.3f}")
            #pmdarima n'affiche pas de critère d'information
            #aic_auto = model_fit.aic
            #bic_auto = model_fit.bic
            #st.write(f"Critère d'Information d'Akaike (AIC) : {aic_auto:.3f}")
            #st.write(f"Critère d'Information Bayésien (BIC) : {bic_auto:.3f}")
